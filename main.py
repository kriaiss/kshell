...
from PyQt6.QtGui import (QTextCursor, QFont, QColor, QTextCharFormat, QIcon)
...
class LaunchpadTab(QWidget):
    ...
    def load_apps(self):
                        ...
                        try:
                            for sub_item in os.listdir(full_path):
                                if sub_item.endswith(".app") and not sub_item.startswith("."):
                                    ...
                                    name = sub_item.replace(".app", "")
                                    self.apps.append({"name": name, "path": sub_path})
        ...
        self.apps = sorted(unique_apps, key=lambda x: x['name'].lower())
    ...
    def eventFilter(self, obj, event):
        if obj == self.input and event.type() == QEvent.Type.KeyPress:
            if event.key() == Qt.Key.Key_Return:
                # Берем самый первый виджет в лейауте (это кнопка приложения)
...
class SearchTab(QWidget):
    def __init__(self):
        ...
        self.cur_idx = 0
        self.title = QLabel(">Google"); self.title.setStyleSheet("font-size: 48px; font-weight: bold; margin-bottom: 2px;")
        self.hint = QLabel("change it on tab")
        self.hint.setObjectName("SearchHint")
        self.input = QLineEdit(); self.input.setPlaceholderText("search ready..."); self.input.setFixedWidth(450)
        layout.addWidget(self.title, 0, Qt.AlignmentFlag.AlignCenter); layout.addWidget(self.hint, 0, Qt.AlignmentFlag.AlignCenter); layout.addWidget(self.input, 0, Qt.AlignmentFlag.AlignCenter)
        self.input.installEventFilter(self)
    ...
    def eventFilter(self, obj, event):
        if event.type() == QEvent.Type.KeyPress:
            ...
            if event.key() == Qt.Key.Key_Return and self.input.text():
                query = urllib.parse.quote(self.input.text())
                webbrowser.open(self.engines[self.cur_idx][1] + query)
...
class ClipboardTab(QWidget):
    ...
    def refresh_list(self):
        ...
        for original_text in display_items[:20]:
            item = QFrame()
            item.setStyleSheet("background: transparent;")
            l = QHBoxLayout(item)
            l.setContentsMargins(10, 5, 10, 5)
            
            preview_text = " ".join(original_text.split())
            if len(preview_text) > 65:
                preview_text = preview_text[:65] + "..."
                
            txt = QLabel(preview_text)
            txt.setObjectName("ClipLabel") 
            
            btn_copy = QPushButton("copy")
            btn_copy.setObjectName("ClipCopyBtn")
            btn_copy.setCursor(Qt.CursorShape.PointingHandCursor)
            btn_copy.clicked.connect(lambda ch, x=original_text: self.copy_and_hide(x))
...
class KShell(QMainWindow):
    def __init__(self):
        ...
        self.end_pos = QPoint(screen_geo.center().x() - 390, screen_geo.center().y() - 250)
        self.start_pos = QPoint(self.end_pos.x(), self.end_pos.y() - 40)
        self.move(self.start_pos); self.setWindowOpacity(0.0)
        self.root = QWidget(); self.root.setObjectName("MainContainer")
        self.main_layout = QVBoxLayout(self.root); self.main_layout.setContentsMargins(10, 10, 10, 15)
        self.stack = QStackedWidget()
        self.term = TerminalTab(self)
        self.stack.addWidget(self.term)
        self.launchpad = LaunchpadTab()
        self.stack.addWidget(self.launchpad)
        self.stack.addWidget(SearchTab())
        self.stack.addWidget(ClipboardTab())
        self.nav_layout = QHBoxLayout(); self.nav_layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.nav_btns = []
        for i, name in enumerate(["terminal", "launchpad", "search", "clipbored"]):
            btn = QPushButton(name); btn.setObjectName("NavBtn"); btn.setCheckable(True)
            btn.clicked.connect(lambda ch, idx=i: self.set_tab(idx))
            self.nav_layout.addWidget(btn); self.nav_btns.append(btn)
        self.main_layout.addWidget(self.stack); self.main_layout.addLayout(self.nav_layout)
        self.setCentralWidget(self.root); self.update_system_theme(); self.set_tab(0)
        self.anim_group = QParallelAnimationGroup()
        self.pos_anim = QPropertyAnimation(self, b"pos")
        self.opacity_anim = QPropertyAnimation(self, b"windowOpacity")
        for a in [self.pos_anim, self.opacity_anim]:
            a.setDuration(150); a.setEasingCurve(QEasingCurve.Type.OutCubic); self.anim_group.addAnimation(a)
        ...
        if event.type() == QEvent.Type.KeyPress:
            if event.modifiers() == Qt.KeyboardModifier.ControlModifier:
                if Qt.Key.Key_1 <= event.key() <= Qt.Key.Key_4:
                    self.set_tab(event.key() - Qt.Key.Key_1); return True
            if event.key() == Qt.Key.Key_Escape: self.toggle(); return True
        return super().eventFilter(obj, event)

    def toggle(self):
        if self.anim_group.state() == QParallelAnimationGroup.State.Running: return
        
        if not self.isVisible() or not NSApp.isActive() or self.windowOpacity() < 0.5:
            self.is_hiding = False
            NSApp.unhide_(None)
            self.show()
    ...
    def update_system_theme(self):
        # Проверяем реальное состояние macOS
        is_dark = NSUserDefaults.standardUserDefaults().stringForKey_("AppleInterfaceStyle") == "Dark"
        # Обновляем QSS
        self.setStyleSheet(get_theme(is_dark))
        # Обновляем терминал (чтобы курсор тоже пересчитался)
        self.term.update_display()

    def changeEvent(self, event):
        if event.type() == QEvent.Type.PaletteChange:
            self.update_system_theme()
            clip_tab = self.stack.widget(3)
            if hasattr(clip_tab, "refresh_list"):
                clip_tab.refresh_list()
...
if __name__ == "__main__":
    ...
    def hotkey_handler(event):
        if event.keyCode() == 49 and event.modifierFlags() & (1 << 20):
            QTimer.singleShot(0, shell.toggle); return None
        return event
    NSEvent.addGlobalMonitorForEventsMatchingMask_handler_(NSKeyDownMask, hotkey_handler)
    NSEvent.addLocalMonitorForEventsMatchingMask_handler_(NSKeyDownMask, hotkey_handler)
    sys.exit(app.exec())
